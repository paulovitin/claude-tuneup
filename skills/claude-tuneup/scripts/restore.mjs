...
function apply(rp) {
  ...
  // 2. Removed items back to original paths.
  const map = readJSON(path.join(rp, 'removed.json')) || {};
  for (const [stashed, original] of Object.entries(map)) {
    if (exists(stashed)) { move(stashed, original); restored.push(`item: ${original}`); }
  }
  ...
  process.stdout.write(JSON.stringify({ restored, manualReAdd: readd }, null, 2) + '\n');
}
