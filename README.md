...
- **↩️ Every run is undoable.** Configs are snapshotted and removed items are *moved* (never `rm`-ed) into a backup under `~/.claude-tuneup/backups/<run-id>/` — kept **outside** the skill dir so a skill update or reinstall can't wipe your undo history (override with `$CLAUDE_TUNEUP_STATE`). Roll back anytime with `claude-tuneup restore`.
